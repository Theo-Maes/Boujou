...
const EventForm = ({
  ...
  const initialStep = groupId ? CHOICE_STEPS : INTRO_EVENT_STEPS;
  const mainStep = groupId ? [] : EVENT_STEPS;
  const successStep = groupId ? [] : SUCCESS_STEPS;
  function trst() {
    //si groupId
  }
  ...
  useEffect(() => {
    if (pages[currentPage].isDark) {
      setIsDark(true);
    } else {
      setIsDark(false);
    }
  }, [pages, currentPage, setIsDark]);
    ...
    try {
      if (groupId) {
        if (
          ...
          await pages[currentPage].onSubmit?.(data, userId, groupId, setError);
          if (currentPage === pages.length - 1) {
            successCallBack?.();
            setIsOpen(false);
          }
        } else if (!pages[currentPage].transition) {
          await pages[currentPage].onSubmit?.(data, userId, groupId);
        }
